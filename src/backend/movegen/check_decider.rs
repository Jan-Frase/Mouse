...
pub fn is_in_check_on_square(state: &State, side: Side, king_square: Square) -> bool {
    let friendly_bb = state.bb_manager.get_all_pieces_bb_off(side);
    let enemy_bb = state
        .bb_manager
        .get_all_pieces_bb_off(side.opposite());
    ...
    for piece_type in ALL_PIECES {
        ...
        let attack_bb = match piece_type {
            ...
            Queen => get_slider_moves_at_square::<true>(king_square, friendly_bb, enemy_bb) | get_slider_moves_at_square::<false>(king_square, friendly_bb, enemy_bb),
        };
